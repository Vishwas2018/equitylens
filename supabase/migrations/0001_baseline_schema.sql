-- ============================================================================
-- EquityLens — Baseline Schema  (migration 0001)
...
-- pg_partman managed by Supabase dashboard; skip in CI dry-run if unavailable
DO $$ BEGIN
  CREATE EXTENSION IF NOT EXISTS pg_partman;
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'pg_partman not available (CI dry-run): %', SQLERRM;
END $$;
...
-- Bootstrap partition management; fall back to a default partition when pg_partman unavailable
DO $$ BEGIN
...
EXCEPTION WHEN OTHERS THEN
  EXECUTE 'CREATE TABLE IF NOT EXISTS scenario_results_default
           PARTITION OF scenario_results DEFAULT';
  RAISE NOTICE 'partman unavailable for scenario_results — created default partition: %', SQLERRM;
END $$;
...
EXCEPTION WHEN OTHERS THEN
  EXECUTE 'CREATE TABLE IF NOT EXISTS audit_logs_default
           PARTITION OF audit_logs DEFAULT';
  RAISE NOTICE 'partman unavailable for audit_logs — created default partition: %', SQLERRM;
END $$;
...
-- ===========================================================================
-- 13. MATERIALISED VIEW: portfolio summary
-- ===========================================================================
