...
export const ENTITLEMENTS: Record<'free' | 'pro' | 'professional',
                                  Record<Entitlement, Quota>> = {
  free: {
...
> *EquityLens provides decision-support tools only. Outputs are not financial, tax, or legal advice. Estimates rely on user-supplied inputs and versioned Australian tax rules. Always consult a registered tax agent or financial adviser before acting on results.*
