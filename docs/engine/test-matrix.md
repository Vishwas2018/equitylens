...
describe.each(fixtures)('ATO/SRO fixture: $name', (fixture) => {
  it(`matches expected outputs (source: ${fixture.source})`, async () => {
    ...
    expect(result.summary.totalTaxOverHorizonCents)
      .toBe(BigInt(fixture.expectedOutputs.totalTaxCents));
...
50 hand-picked production-shaped scenarios live in `/engine/__tests__/golden/*.json`. Each contains inputs + expected outputs from the *current* engine release. On every PR, the engine is run against each and any deviation > $0.01 in any monetary field fails the build.
