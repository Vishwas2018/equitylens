...
| BL-0022 | Migrate Next.js 14.2.29 → 15.x                                                                             | P0       | M      | DEF-0001                                           | done     | Day 13, D13-T4. Next.js 15.5.18; async cookies/headers/params; engine build tsconfig fixed. Full suite green (engine 421, web 94). Commit `70fe9bc`.                                                                                      |
...
| BL-0029 | Postgres RLS isolation integration tests — cross-tenant JWT probe against real Supabase                    | P0       | S      | D08-T4 (app-layer scoping only)                    | done     | Day 13, D13-T2/T3. Technique A (SET LOCAL): 14 tests, all D9–D12 tables. Technique B (JWT probe, no app filter): 4 tests. All green. Commits `29551ea`, `495ef30`.                                                                        |
| BL-0030 | OpenAI fallback in AI gateway — structural stub, never functionally exercised                              | P1       | S      | D11-T3 (Q1=A decision)                             | done     | D14-T4. `callOpenAiFallback` implemented (gpt-4o-mini tool-calling). 2 always-run tests green (mock Anthropic → fallback_used=true logged → ok:false). 3 live-OpenAI tests skipped (no OPENAI_API_KEY) → D15-T0 to provision key and run. |
| BL-0031 | Stripe dunning — retry logic, grace-period state machine, failure emails                                   | P2       | M      | D14 plan — approved slip                           | proposed | Post-RC. Requires Stripe retry dashboard config + grace-period state machine. Zero RC-blocking user-facing value.                                                                                                                         |
| BL-0032 | Live Stripe checkout path untested in CI — skipIf fired, STRIPE_SECRET_KEY absent from CI secrets          | P1       | XS     | D14-T3 — 1 skipped test                            | proposed | Wire STRIPE_SECRET_KEY (test-mode) + STRIPE_WEBHOOK_SECRET to GitHub Secrets → Actions; add to test job env. Sig-verification always runs; this gap is API call only.                                                                     |
...
- **Priority**: P1
- **Effort**: S (2–4h)
- **Status**: done (live-path verification → D15-T0)
...
**D14-T4 outcome**: `callOpenAiFallback` now makes a real OpenAI call (`gpt-4o-mini`, tool-calling with same schema as Anthropic path). Returns `Explanation | null`. Grounding gate applied on the returned value before it reaches the caller. `fallback_used` logged to `ai_interactions` on every fallback invocation.

Test file: `apps/web/__tests__/ai-gateway-fallback.test.ts` — 2 always-run tests green, 3 live-OpenAI tests skipped (no `OPENAI_API_KEY` locally or in CI).

**Acceptance criteria**

- [x] `callOpenAiFallback` calls real OpenAI endpoint (gpt-4o-mini tool-calling)
- [x] `fallback_used = true` in `ai_interactions` on fallback path (always-run test verifies)
- [x] Grounding gate still applies on fallback response (code path confirmed; live verification pending)
- [x] No regression in existing unit tests (101/101 + 4 skipped)
- [ ] **D15-T0**: Provision `OPENAI_API_KEY` (test-mode) in CI + staging; 3 skipped live-OpenAI tests must run green before RC walkthrough

**Linked records**: D11-T3 | BL-0029 | BL-0032
